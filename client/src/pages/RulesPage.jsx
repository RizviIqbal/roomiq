...
export default function RulesPage() {
  ...
  const filtered = tab ? rules.filter(r => r.status === tab) : rules
    ...
    <div className="w-full px-6 md:px-[64px] pb-24">
      ...
      <div className="flex gap-3 mb-10 overflow-x-auto pb-2">
        {TABS.map(t => (
          ...
          >
            {t.label}
            {t.value && <span className={['ml-2 px-2 py-0.5 rounded-full text-[9px] bg-obsidian/30', tab === t.value ? 'text-obsidian bg-obsidian/10' : 'text-primary-muted bg-white/10'].join(' ')}>{rules.filter(r=>r.status===t.value).length}</span>}
          </button>
        ))}
      </div>
      ...
      {loading || hLoading ? (
        ...
        <div className="glass-panel rounded-[32px] p-8 max-w-3xl">
          <EmptyState icon="📜" title="No rules found" description={tab ? `NO ${tab.toUpperCase()} RULES` : 'PROPOSE YOUR FIRST HOUSE RULE'} />
        </div>
      ) : (
